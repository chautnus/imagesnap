...
export default function App() {
  ...
  const { appData, isSyncing, refreshData, handleSaveProduct, handleUpdateProduct, handleDeleteProduct, handleSaveCategory, handleDeleteCategory } = useAppData(spreadsheetId, user);
  ...
  // Auth initialization ΓÇö GIS (web) + SYS_AUTH_SUCCESS event (extension)
  useEffect(() => {
  ...
  // Hash navigation + #import= handling (Extension ΓåÆ Web handoff)
  useEffect(() => {
  ...
  if (view === 'privacy' || location.pathname === '/privacy') {
    return (<><SEO title="Privacy Policy ΓÇö ImageSnap.cloud" description="Our privacy policy and data protection commitment." /><PrivacyPolicy onBack={() => { window.location.href = '/'; }} /></>);
  }
  ...
  if (user && isAuthReady) {
    return (
      <AppDashboard
        ...
        handleSaveProduct={handleSaveProduct} handleUpdateProduct={handleUpdateProduct} handleDeleteProduct={handleDeleteProduct}
